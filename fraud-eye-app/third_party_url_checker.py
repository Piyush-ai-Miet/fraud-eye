    ...
    def check_virustotal(self, url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[VIRUSTOTAL] URL not in database, submitting for scan...")
                return self._submit_url_to_virustotal(url)
            
            elif response.status_code == 429:
                # Rate limit exceeded
                print(f"[VIRUSTOTAL] Rate limit exceeded (4 req/min, 500/day)")
                return {'error': 'VirusTotal rate limit exceeded', 'verdict': 'RATE_LIMITED'}
            
            elif response.status_code == 401:
                # Invalid API key
                print(f"[VIRUSTOTAL] Invalid API key")
                return {'error': 'VirusTotal API key invalid', 'verdict': 'AUTH_ERROR'}
